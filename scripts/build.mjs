...
function markdown(md='') {
  ...
  for (const raw of lines) {
    ...
    if (ul) { flushPara(); if (list && list !== 'ul') flushList(); if (!list) { list='ul'; out.push('<ul>'); } out.push(`<li>${inline(ul[1])}</li>`); continue; }
    const ol = line.match(/^\d+\.\s+(.+)$/);
    if (ol) { flushPara(); if (list && list !== 'ol') flushList(); if (!list) { list='ol'; out.push('<ol>'); } out.push(`<li>${inline(ol[1])}</li>`); continue; }
    const imageOnly = line.match(/^!\[([^\]]*)\]\(([^\s)]+)(?:\s+"([^"]*)")?\)$/);
    if (imageOnly) { flushPara(); flushList(); out.push(`<figure><img src="${escAttr(imageOnly[2])}" alt="${escAttr(imageOnly[1])}" loading="lazy">${imageOnly[3] ? `<figcaption>${esc(imageOnly[3])}</figcaption>` : ''}</figure>`); continue; }
    para.push(line.trim());
  }
...
function readArticles() {
  return fs.readdirSync(contentDir).filter(f=>f.endsWith('.json')).map(file => {
    const a = JSON.parse(fs.readFileSync(path.join(contentDir,file),'utf8'));
...
function card(a) {
  ...
  return `<article class="story-card ${a.featureImage ? 'has-image':''}" data-sb-object-id="${escAttr(a._file)}"><div class="story-copy"><div class="meta"><span data-sb-field-path="category">${esc(a.category||'ARTICLE')}</span><time data-sb-field-path="date" datetime="${escAttr(a.date)}">${esc(a.displayDate)}</time></div><h2 class="story-title" data-sb-field-path="title"><a href="${a.url}">${esc(a.title)}</a></h2><p class="story-excerpt" data-sb-field-path="excerpt">${esc(a.excerpt||'')}</p><div class="story-actions"><a class="action" href="${a.url}">READ MORE →</a><button class="action" type="button" data-share data-share-url="${a.url}" data-share-title="${escAttr(a.title)}">SHARE ↗</button></div></div>${image}</article>`;
}
...
for (let i=0;i<articles.length;i++) {
  ...
  const body=`<main class="article-page"><article data-sb-object-id="${escAttr(a._file)}"><header class="article-header reading"><div class="meta"><span data-sb-field-path="category">${esc(a.category||'ARTICLE')}</span><time data-sb-field-path="date" datetime="${escAttr(a.date)}">${esc(a.displayDate)}</time></div><h1 class="article-title" data-sb-field-path="title">${esc(a.title)}</h1><p class="article-deck" data-sb-field-path="excerpt">${esc(a.excerpt||'')}</p><button class="action" type="button" data-share data-share-url="${a.url}" data-share-title="${escAttr(a.title)}">SHARE ↗</button></header>${feature}<div class="prose reading" data-sb-field-path="body">${markdown(a.body)}</div><footer class="article-footer reading"><button class="action" type="button" data-share data-share-url="${a.url}" data-share-title="${escAttr(a.title)}">SHARE ARTICLE ↗</button></footer>${nextHtml}</article></main>`;
