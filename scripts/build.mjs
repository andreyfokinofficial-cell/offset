...
function shell({title='ОФСЕТ', description='Independent online publication', body, articles, canonical='/'}) {
...
<meta property="og:site_name" content="ОФСЕТ"><meta property="og:title" content="${escAttr(title)}"><meta property="og:description" content="${escAttr(description)}"><meta property="og:type" content="article">
</head><body>
<header class="site-header shell"><a class="masthead" href="/" aria-label="ОФСЕТ — главная">ОФСЕТ</a><button class="search-open" type="button" data-search-open>ПОИСК</button></header>
${body}
<footer class="site-footer shell"><span>ОФСЕТ</span><span>© ${new Date().getFullYear()}</span></footer>
<div class="search-panel" data-search-panel aria-hidden="true"><div class="search-inner shell"><div class="search-top"><span class="search-label">ПОИСК ОФСЕТ</span><button class="action" type="button" data-search-close>ЗАКРЫТЬ ×</button></div><input class="search-input" data-search-input type="search" placeholder="Поиск…" aria-label="Поиск статьи"><div class="search-results" data-search-results></div></div></div>
...
function card(a) {
  ...
  return `<article class="story-card ${a.featureImage ? 'has-image':''}"><div class="story-copy"><div class="meta"><span>${esc(a.category||'ARTICLE')}</span><time datetime="${escAttr(a.date)}">${esc(a.displayDate)}</time></div><h2 class="story-title"><a href="${a.url}">${esc(a.title)}</a></h2><p class="story-excerpt">${esc(a.excerpt||'')}</p><div class="story-actions"><a class="action" href="${a.url}">ЧИТАТЬ ДАЛЕЕ →</a><button class="action" type="button" data-share data-share-url="${a.url}" data-share-title="${escAttr(a.title)}">ПОДЕЛИТЬСЯ ↗</button></div></div>${image}</article>`;
}

fs.rmSync(distDir,{recursive:true,force:true}); fs.mkdirSync(distDir,{recursive:true}); copyDir(publicDir,distDir);
const articles = readArticles();
const home = shell({ title:'ОФСЕТ', description:'Independent online publication', articles, body:`<main class="shell"><section class="index-heading"><span>ПОСЛЕДНЕЕ</span><span>${String(articles.length).padStart(2,'0')} СТАТЬИ</span></section><section class="story-list">${articles.map(card).join('\n')}</section></main>` });
...
for (let i=0;i<articles.length;i++) {
  ...
  const nextHtml=next ? `<aside class="next shell"><span>СЛЕДУЮЩАЯ</span><a href="${next.url}">${esc(next.title)} →</a></aside>`:'';
  const body=`<main class="article-page"><article><header class="article-header reading"><div class="meta"><span>${esc(a.category||'ARTICLE')}</span><time datetime="${escAttr(a.date)}">${esc(a.displayDate)}</time></div><h1 class="article-title">${esc(a.title)}</h1><p class="article-deck">${esc(a.excerpt||'')}</p><button class="action" type="button" data-share data-share-url="${a.url}" data-share-title="${escAttr(a.title)}">ПОДЕЛИТЬСЯ ↗</button></header>${feature}<div class="prose reading">${markdown(a.body)}</div><footer class="article-footer reading"><button class="action" type="button" data-share data-share-url="${a.url}" data-share-title="${escAttr(a.title)}">ПОДЕЛИТЬСЯ СТАТЬЕЙ ↗</button></footer>${nextHtml}</article></main>`;
  fs.writeFileSync(path.join(dir,'index.html'), shell({title:`${a.title} — ОФСЕТ`, description:a.excerpt||'', body, articles, canonical:a.url}));
}

const notFound=shell({title:'404 — ОФСЕТ',description:'Страницы нет',articles,body:`<main class="error-page reading"><div class="meta"><span>404</span></div><h1>Страницы нет.</h1><a class="action" href="/">НАЗАД НА ГЛАВНУЮ →</a></main>`});
